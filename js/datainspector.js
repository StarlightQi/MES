...
Inspector.prototype.inspectObject = function(object) {
  ...
  if (!inspectedObjects || !this.multipleSelection) {
    ...
    // Go through all the properties passed in to the inspector and show them, if appropriate:  将所有属性传递给检查员，并在适当的情况下显示给他们
    ...
    // Go through all the properties on the model data and show them, if appropriate:通过模型数据上的所有属性并显示他们，如果合适的话
    ...
    var keyName = $('.inspector>table>tbody>tr:first>td:first').text();
    var remarkName = $('.inspector>table>tbody>tr').eq(1).find('td:first').text();
    var objName = $('.inspector>table>tbody>tr:last>td:first').text();
    if (keyName == 'text') {
        $('.inspector>table>tbody>tr>td:first').text('姓名')
    }
    if (remarkName == 'remark') {
        $('.inspector>table>tbody>tr').eq(1).find('td:first').text('备注');
    }
    if (objName == 'obj') {
        $('.inspector>table>tbody>tr:last>td:first').text('处理人')
    }

    //判断节点修改数据
    var nodeType = data.category;   //只有是 节点 与 判断时 才可以编辑
    if(nodeType != 'Default' || nodeType != 'Conditional'){
        $('.inspector>table>tbody>tr:last').css({'display':'none'});
    }

    /****************************如果是节点和判断**************************************/
    if(nodeType == 'Default' || nodeType == 'Conditional') {
        $('.inspector>table>tbody>tr:last').css({'display':''});
          var creUlCon = document.createElement('div');
          creUlCon.className = 'creUlCon';
          var crepT = document.createElement('p');
          crepT.className = 'noticeText';
          var creSpan = document.createElement('span');
          creSpan.innerHTML = '待选区';
          var creSpanM = document.createElement('span');
          creSpanM.innerHTML = '已选区';
          crepT.appendChild(creSpan);
          crepT.appendChild(creSpanM);
          creUlCon.appendChild(crepT);
/**********************创建处理人框********************************************/
          var creUlL = document.createElement('ul');
          creUlL.className = 'creUlLclass';
          var creUlR = document.createElement('ul');
          creUlR.className = 'creUlRclass';
          creUlL.innerHTML = null;
          creUlR.innerHTML = null;

          //读取数据填充

          //已选信息
          var objType = data.obj;
          var haveChoice = data.choice;
          var haveChoiceLength = data.choice.length;  /*已经选择处理人*/
          

          var haveValue = [];
          var haveName = [];
          for(var i=0; i<haveChoiceLength; i++){   //整理已选信息
              haveValue.push(haveChoice[i].value);
              haveName.push(haveChoice[i].name);
          }

          //未选信息
          var unHaveValue = [];
          var unHaveName = [];
          if(objType == '个人'){  //整理个人未选信息
              var unChoice = data.personAll; //所有可选人
              var unChoiceLength = unChoice.length;
              for(var i=0; i<unChoiceLength; i++){
                  var undetermined = unChoice[i].value;
                  var undeterminedName = unChoice[i].name;
                  if(haveValue.indexOf(undetermined)<0){
                      unHaveValue.push(undetermined);
                  }
                  if(haveName.indexOf(undeterminedName)<0){
                      unHaveName.push(undeterminedName);
                  }
              }
          }else if(objType == '部门'){  //整理部门未选信息
              var unChoice = data.departmentAll; //所有可选人
              var unChoiceLength = unChoice.length;
              for(var i=0; i<unChoiceLength; i++) {
                  var undetermined = unChoice[i].value;
                  var undeterminedName = unChoice[i].name;
          ...
          // 将未选信息放入左侧
          var unHaveValueLength = unHaveValue.length;
          for(var i=0;i<unHaveValueLength; i++){
              var creUnLi = document.createElement('li');
              creUnLi.setAttribute('value',unHaveValue[i]);
              creUnLi.innerHTML = unHaveName[i];
              if(unHaveName[i] != undefined){
                  creUlL.appendChild(creUnLi);
              }
          }

          //创建已选区域
          var dataSelected = data.choice;
          var dataSelectedLength = dataSelected.length;
        /**********************************初始化处理人已选长度判断***********************************/
          /*if(dataSelectedLength < 1) {
            creUlR.style.cssText = 'border:1px solid red';
          };*/
          for(var i=0; i< dataSelectedLength; i++){
              var creLi = document.createElement('li');
              creLi.setAttribute('value',dataSelected[i].value);
              creLi.innerHTML = dataSelected[i].name;
              creUlR.appendChild(creLi);
          }


          /*处理类别更换信息*/

          var handleType = $('.inspector>table>tbody>tr:last>td:last>select');
          handleType.change(function(){
            var changeText = $(this).find('option:checked').text();
            creUlL.innerHTML = null;
            creUlR.innerHTML = null;
 /************************************处理对象改变，已选取清空红框显示*********************************************/
           /* creUlR.style.cssText = 'border:1px solid red';*/
            data.choice = [];
            if(changeText == '个人'){
              var personAll = data.personAll;
              var personAllLength = personAll.length;
              for(var i=0; i<personAllLength; i++){
                var creLi = document.createElement('li');
                creLi.setAttribute('value',personAll[i].value);
                creLi.innerHTML = personAll[i].name;
                creUlL.appendChild(creLi);
              }
            }else if(changeText == '部门'){
              var departmentAll = data.departmentAll;
              var departmentAllLength = departmentAll.length;
              for (var i = 0; i < departmentAllLength; i++) {
                  var creLi = document.createElement('li');
                  creLi.setAttribute('value', departmentAll[i].value);
                  creLi.innerHTML = departmentAll[i].name;
                  creUlL.appendChild(creLi);
              }
            }
          });

          creUlCon.appendChild(creUlL);
          creUlCon.appendChild(creUlR);
          mainDiv.appendChild(creUlCon);
        /*添加移除数据*/
        /****************************待选区与已选区来回点击选择*****************************************/
        mainDiv.onclick = function(e) {
            ...
            if (tagName == 'LI') {
                var parentName = target.parentNode.className;
                if (parentName == 'creUlLclass') {   //添加到已选区域
                    target.parentNode.removeChild(target);
                    creUlR.appendChild(target);
   /***********************************只要点击就会有已选择信息，去掉红框**************************************************/
                    creUlR.style.cssText = 'border:none';
                } else if (parentName == 'creUlRclass') {    //移除已选择区域的项目
                    target.parentNode.removeChild(target);
 /**************************************点击选择判断已选区长度**********************************************/
                    var length = $('.creUlRclass').children().length;
                    /*if(length < 1) {
                      creUlR.style.cssText = 'border:1px solid red';
                    }*/
                    creUlL.appendChild(target);


                }
            }

            var creUlRLength = creUlR.children.length;
            var newChoice = [];
            for (var i = 0; i < creUlRLength; i++) {
                var newObj = {};
                newObj.value = creUlR.children[i].getAttribute('value');
                newObj.name = creUlR.children[i].innerHTML;
                newChoice.push(newObj);
            }
  ...
  } else { // multiple objects selected   选择多个对象
    var mainDiv = this._div;
    mainDiv.innerHTML = '';
    var shared = new go.Map(); // for properties that the nodes have in common 对于节点具有共同属性的
    var properties = new go.Map(); // for adding properties   添加属性
    var all = new go.Map(); // used later to prevent changing properties when unneeded   在不需要时用于防止更改属性
    ...
    if (data) { // initial pass to set shared and all
      ...
      // Go through all the properties on the model data and add them to the map, if appropriate:  通过模型数据上的所有属性，并将它们添加到映射中，如果合适的话
      if (this.includesOwnProperties) {
        for (var k in data) {
          if (k === '__gohashid') continue; // skip internal GoJS hash property
          if (this._inspectedProperties[k]) continue; // already exists
